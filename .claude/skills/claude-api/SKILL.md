---
name: claude-api
description: "Build apps with the Claude API or Anthropic SDK. TRIGGER when: code imports `anthropic`/`@anthropic-ai/sdk`/`claude_agent_sdk`, user asks how to call the Claude API, set up authentication, handle requests/responses/errors, or use the Anthropic SDK in any language. Also triggers for Agent SDK questions. DO NOT TRIGGER when: code imports `openai`/other AI SDK, general programming unrelated to Claude, or ML/data-science tasks."
license: Complete terms in LICENSE.txt
---

# Claude API — Setup, Auth, Requests, and Best Practices

This skill covers everything needed to build with the Claude API: authentication, SDK installation, making requests, handling responses, debugging errors, and choosing between API surfaces. Detect the project language, then read the relevant language-specific documentation.

## Defaults

Unless the user requests otherwise:

- **Model:** Use `claude-opus-4-6`. Never downgrade to Sonnet or Haiku — cost decisions belong to the user.
- **Thinking:** Use `thinking: {type: "adaptive"}` for anything non-trivial. Do not use `budget_tokens` on Opus 4.6 or Sonnet 4.6 — it is deprecated.
- **Streaming:** Default to streaming for long inputs, long outputs, or high `max_tokens`. Use `.get_final_message()` / `.finalMessage()` if you don't need individual stream events.

---

## Authentication

Every API request requires an API key. Set it as the environment variable `ANTHROPIC_API_KEY` — never hardcode keys in source files.
...
If no API key is set, the SDK throws `AuthenticationError` (HTTP 401). Get a key at console.anthropic.com.
...
| HTTP | Error class | Cause | Fix |
|------|-------------|-------|-----|
| 401 | `AuthenticationError` | Missing or invalid API key | Check `ANTHROPIC_API_KEY` is set |
| 400 | `BadRequestError` | Invalid parameters (e.g., `budget_tokens` on Opus 4.6, prefill on Opus 4.6, wrong `output_config`) | Read the error message — it names the bad field |
| 429 | `RateLimitError` | Too many requests | Retry with exponential backoff; check tier limits |
| 529 | `OverloadedError` | Anthropic overloaded | Retry with backoff |
| 500 | `InternalServerError` | Server-side error | Retry once; if persistent, check status.anthropic.com |
...
Use typed exception classes (`anthropic.RateLimitError`, `anthropic.AuthenticationError`, etc.) — never match error strings.

For the full error code reference, read `shared/error-codes.md`.
...
- **Always set `max_tokens` explicitly** — the default is very low and will truncate output. Use `~1024` for short answers, `~16000` for general use, `~64000` for streaming long responses.
- **Use streaming for long responses** — prevents HTTP timeouts. `.stream()` + `.get_final_message()` / `.finalMessage()` gives you both.
- **Use SDK types** — `Anthropic.MessageParam`, `Anthropic.Tool`, `Anthropic.Message`, etc. Don't redefine what the SDK already exports.
- **Use SDK helpers** — `stream.finalMessage()` instead of wrapping `.on()` events in `new Promise()`.
- **Use `output_config.format`** for structured outputs — not the deprecated `output_format` parameter.
- **Use `thinking: {type: "adaptive"}`** on Opus 4.6/Sonnet 4.6. Never use `budget_tokens` on these models.
- **Don't truncate file content** — if input is too long for context, tell the user and discuss chunking or summarization.
