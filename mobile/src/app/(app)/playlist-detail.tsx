...
import { TOTAL_BOTTOM_OFFSET } from '@/constants/Layout';
  ...
  return (
    <>
      <View style={{ width: SCREEN_WIDTH, height: HERO_HEIGHT, overflow: 'hidden', backgroundColor: '#000' }}>
        ...
        <Animated.View
          pointerEvents="none"
          style={[StyleSheet.absoluteFillObject, heroBlurStyle]}
        >
...
export default function PlaylistDetailScreen() {
  ...
  const flashListData = useMemo(
    () => buildPlaylistDetailFlashRows(tracks, playlistTitleForAds),
    [tracks, playlistTitleForAds],
  );
  ...
  /** Dock: tab bar + mini — flush last row above chrome; + home-indicator inset. */
  const listPadBottom = TOTAL_BOTTOM_OFFSET + 20 + insets.bottom;
              ...
              {recommended.length > 0 ? (
                ...
                >
                  {recDownloadingAll ? (
                    ...
                    <Download size={18} color="rgba(255,255,255,0.88)" strokeWidth={2.2} />
                  )}
                </Pressable>
              ) : null}
            </View>
