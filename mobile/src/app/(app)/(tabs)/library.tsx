...
import { View, Text, ScrollView, Pressable, Modal, Dimensions, TextInput, KeyboardAvoidingView, Platform, Keyboard, StyleSheet } from 'react-native';
...
/** Mini-player + tab bar clearance (explicit vault spec). */
const LIBRARY_SCROLL_PADDING_BOTTOM = 180;
...
export default function LibraryScreen() {
                  ...
                  {album.tracks.map((track, i) => (
                    ...
                    >
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={{ color: 'rgba(255,255,255,0.5)', fontSize: 12, marginTop: 1 }} numberOfLines={1}>{track.artist}</Text>
                      </View>
                    </Pressable>
                  ))}
                </View>
          ...
          >
            ...
            >
              ...
              <View className="flex-1 ml-3">
                <Text className="text-white font-semibold text-sm">Import to Vault</Text>
                <Text className="text-white/70 text-xs">Command ingest · device files</Text>
              </View>
                ...
                >
                  ...
                  <View className="flex-1 ml-4">
                    ...
                    <View className="flex-row items-center mt-0.5">
                      ...
                      <Text style={{ color: 'rgba(255,255,255,0.5)', fontSize: 14, marginLeft: 4 }}>{track.artist}</Text>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#000000' }}>
      {/* Header */}
      <View className="px-4 py-3" style={{ paddingTop: insets.top + 24 }}>
        <View className="flex-row items-center justify-between mb-4">
          <Text className="text-white text-[22px] font-bold pl-1">Your Library</Text>
          <Pressable
                ...
                style={[
                  ...
                  isOn
                    ? Platform.select({
                        ...
                        android: { borderColor: 'rgba(255,0,255,0.7)', elevation: 6 },
                        default: { borderColor: 'rgba(255,0,255,0.55)' },
                      })
                    : null,
                ]}
              >
                <Text style={{ color: '#fff', fontSize: 13, fontWeight: isOn ? '800' : '500' }}>
                  {filter.label}
                </Text>
              </Pressable>
            );
      ...
      {showLibraryHomeList ? (
        ...
        <FlashList
          ...
          contentContainerStyle={{ paddingBottom: LIBRARY_SCROLL_PADDING_BOTTOM }}
            ...
            return (
              ...
              >
                {item.track.artwork ? (
                  ...
                  <View style={{ width: 48, height: 48, borderRadius: 12, borderWidth: 1, borderColor: 'rgba(255,255,255,0.12)', backgroundColor: 'rgba(139,92,246,0.15)', alignItems: 'center', justifyContent: 'center' }}>
                    <Heart size={20} color="#8B5CF6" />
                  </View>
                )}
                <View style={{ flex: 1, marginLeft: 12 }}>
                  <Text style={{ color: '#fff', fontWeight: '600', fontSize: 14 }} numberOfLines={1}>{item.track.title}</Text>
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 2, gap: 6 }}>
                    {getSourceIcon(item.track.source ?? '')}
                    <Text style={{ color: 'rgba(255,255,255,0.5)', fontSize: 12, flex: 1 }} numberOfLines={1}>{item.track.artist}</Text>
                  </View>
                </View>
              </Pressable>
            );
