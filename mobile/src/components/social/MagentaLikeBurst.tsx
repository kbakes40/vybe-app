...
/**
 * Neon cyan heart — Reanimated spring, Success haptic on like, hold-glow pulse, particle burst.
 */
export function MagentaLikeBurst({ liked, onToggle, iconSize = 22 }: Props) {
  ...
  const heartGlowStyle = useAnimatedStyle(() => {
    ...
    return {
      ...
      ...(IS_ANDROID ? { elevation: baseEl + g * 8 } : {}),
    };
  });
  ...
  return (
    ...
    >
      ...
      <Animated.View style={[heartGlowStyle, heartScaleStyle, styles.heartCenter]}>
        <Heart
          size={iconSize}
          color={liked ? VIBRANT_BLUE : 'rgba(255,255,255,0.85)'}
          fill={liked ? VIBRANT_BLUE : 'transparent'}
        />
      </Animated.View>
    </Pressable>
  );
