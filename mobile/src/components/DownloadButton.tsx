...
import { View, Pressable, Animated, ActivityIndicator, Dimensions } from 'react-native';
...
export function DownloadButton({ track, size = 28, onDownloadComplete }: DownloadButtonProps) {
  ...
  const renderContent = () => {
    ...
    if (isDownloading) {
      if (progress < 0.02) {
        return (
          <ActivityIndicator size="small" color="#3B82F6" style={{ width: size, height: size }} />
        );
      }
    ...
    return (
      ...
      >
        <View style={{
          ...
          <View style={{ alignItems: 'center' }}>
            <View style={{ width: 1.5, height: size * 0.27, backgroundColor: 'rgba(255,255,255,0.75)', borderRadius: 1 }} />
            <View style={{
              width: 0, height: 0,
              borderLeftWidth: size * 0.18, borderRightWidth: size * 0.18, borderTopWidth: size * 0.18,
              borderLeftColor: 'transparent', borderRightColor: 'transparent',
